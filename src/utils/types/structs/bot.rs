...
pub struct Bot {
    ...
    pub retry_oracle: Arc<Mutex<RetryOracle>>,
    pub fork_db_oracle: Arc<Mutex<ForkOracle>>,
}
...
impl Bot {
    ...
    pub fn new(
        ...
        fork_db_oracle: Arc<Mutex<ForkOracle>>
    ) -> Self {
        Bot {
            block_oracle,
            nonce_oracle,
    ...
    pub async fn get_fork_db(&self) -> ForkDB {
        let fork_oracle = self.fork_db_oracle.lock().await;
    ...
    pub async fn add_tx_to_retry_oracle(&mut self, tx_data: SnipeTx) {
        let mut retry_oracle = self.retry_oracle.lock().await;
    ...
    pub async fn get_retry_oracle_tx_data(&self) -> Vec<SnipeTx> {
        let retry_oracle = self.retry_oracle.lock().await;
